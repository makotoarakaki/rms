	...
	public function add($id = null) {
		...
		if ($this->request->is('post')) {
			...
			if ($this->HumanCost->save($this->request->data)) {
				$this->Session->setFlash(__('人件費が保存されました。'));
				$this->redirect(array('action' => 'index'.'/'.$id));
			} else {
				$this->Session->setFlash(__('人件費は保存されませんでした。もう一度やり直して下さい。'));
			}
		}
		$this->set('staff', $info);
	}
 ...
 */
	public function delete($id = null) {
		...
		if ($this->HumanCost->delete()) {
			$this->Session->setFlash(__('人件費が削除されました。'));
			$this->redirect(array('action' => 'index'));
		}
		$this->Session->setFlash(__('人件費は削除されませんでした。'));
		$this->redirect(array('action' => 'index'));
	}
}
