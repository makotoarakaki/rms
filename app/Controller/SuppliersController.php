 ...
 */
	public function add() {
			...
			if (!empty($this->data)) {
				...
				foreach ($this->data['Supplier'] as $data) {
					...
					if (isset($data['item_id'])) {
						...
						// 合計値を取得
						$total = $this->_unit_total($data['item_count'], $data['cost'], $data['tax']);
						// 合計値のリストを追加
		...
		$options['fields'] = array('MainItem.id AS main_item_id', 'MainItem.main_item_name AS main_item_name',
				                   'Item.id AS item_id', 'Item.item_name AS item_name', 'Item.cost AS cost', 'Item.tax AS tax', 'Item.no_cost AS no_cost');
	...
	public function edit($id = null, $item_id = null) {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if ($data[0]['Item']['no_cost'] == 0) {
				...
				$tax = $data[0]['Item']['tax'];
				// 仕入れ単価 × 数量
				$total = $cost * $this->request->data['Supplier']['item_count'];
				// 消費税の算出
				$total = $total + ($total * $tax / 100);
				$this->request->data['Supplier']['total'] = $total;
			}
 ...
 */
     ...
     * @return $total 合計値
     */
    private function _unit_total($item_count = 0, $cost = 0, $tax = 0) {
    	$total = 0;
    	if ($item_count != 0) {
